...
ui <- fluidPage(
  ...
  sidebarPanel(h4("Telecom APP on Google Play Store Rivew"),
               selectInput("telecom",label = "choose a telecom",choices = telecom),
               selectInput('content',label = "choose a comment",choices = cont),
               dateRangeInput("dates", label = h5("Date range"),start = min(total_review$repdate),end = Sys.time())),
  mainPanel(tabsetPanel(type = "tabs",
                        tabPanel("Plot", wordcloud2Output('plot',height = 800)),
                        tabPanel("Stats", verbatimTextOutput("stats"))))
)
server <- function(input, output) {
  output$stats <- renderPrint(ha(input$content,input$telecom,input$dates[1],input$dates[2]))
  output$plot <- renderWordcloud2(
    wordcloud2(haha(input$content,input$telecom,input$dates[1],input$dates[2]),
